...
# Aula 15 — API completa (consolidação).
...
def validar_produto(nome, preco, marca, estoque, descricao):
    ...
    if estoque is None:
        ...
        erros["estoque"] = "O estoque não deve ser negativo"
...
def listar_produtos(
    ...
    campos_ordenacao = ["nome", "preco", "marca", "estoque", "descricao"]
    ...
    if search is not None:
        termo = search.lower()
        resultado = [p for p in resultado if termo in p["nome"].lower() or (p.get("marca") and termo in p["marca"].lower()) or (p.get("descricao") and termo in p["descricao"].lower())]
...
def criar_produto(produto: ProdutoInput):
    ...
    erros = validar_produto(nome_limpo, produto.preco, marca_limpa, produto.estoque, produto.descricao)
    if erros:
        raise HTTPException(status_code=400, detail=erros)

    novo_id = max([item["id"] for item in produtos], default=0) + 1
    novo_produto = {"id": novo_id, "nome": nome_limpo, "preco": produto.preco, "marca": marca_limpa, "estoque": produto.estoque, "descricao": produto.descricao}
...
def atualizar_produto(id: int, produto: ProdutoInput):
    for index, item in enumerate(produtos):
        if item["id"] == id:
            ...
            if erros:
                raise HTTPException(status_code=400, detail=erros)
            produtos[index] = {"id": id, "nome": nome_limpo, "preco": produto.preco, "marca": marca_limpa, "estoque": produto.estoque, "descricao": produto.descricao}
            salvar_produtos(produtos)
